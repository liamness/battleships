...
function checkNotIntersecting(otherShip) {
  // find extents
  const shipOneExtents = this.getExtents();
  const shipTwoExtents = otherShip.getExtents();
...
export default class {
  ...
  move(position) {
    ...
    if (x < 0) {
      x = 0;
    } else if (x > constants.gridSize - this.size[0]) {
      x = constants.gridSize - this.size[0];
    }

    if (y < 0) {
      y = 0;
    } else if (y > constants.gridSize - this.size[1]) {
      y = constants.gridSize - this.size[1];
    }
  ...
  rotate() {
    const diffX = Math.round((this.size[0] + this.size[1]) / 2) - 1;
    const newPosition = [
